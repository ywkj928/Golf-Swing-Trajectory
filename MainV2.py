...
from Filter import ParticleFilter, KalmanFilter  # 입자 필터, 칼만 필터
        ...
        while self.cap.isOpened():
            ...
            for box, track_id, cls_id in zip(boxes, track_ids, cls_ids):
                ...
                if track_id in hand_positions:
                    self.particle_filter.update((center_x, center_y))  # 입자 필터 업데이트
                    # 칼만 필터 업데이트
                    # 칼만 필터에 [x, y, vx, vy]를 전달해야 하므로, 속도도 추정값으로 추가
                    vx, vy = 0, 0  # 현재는 속도를 0으로 가정, 이후에 속도 추정 로직 추가 가능
                    self.kalman_filter.update(np.array([center_x, center_y, vx, vy]))  # 칼만 필터 업데이트
